import { coordinatesMatch, stopCoordinate, type MapCoordinate } from '@/features/minibus/lib/stops';
...
export function buildJourneySteps(
  ...
  journey.legs.forEach((leg, legIndex) => {
    ...
    steps.push({
      ...
      detail: t('minibusStepBoardDetail', { stop: leg.board.name, line: directionsLineLabel(leg) }),
    ...
    steps.push({
      ...
      detail: t('minibusStepAlightDetail', { stop: leg.alight.name, line: directionsLineLabel(leg) }),
      accent: leg.line_color ?? undefined,
...
export function journeyMapMarkers(steps: JourneyStep[]): JourneyMapMarker[] {
  ...
  for (const step of steps) {
    ...
    const color = step.kind === 'transfer' ? JOURNEY_TRANSFER_MARKER_COLOR : (step.accent ?? '#2563eb');
...
function findJourneyMapMarker(markers: JourneyMapMarker[], stepKey: string): JourneyMapMarker | null {
  return markers.find((marker) => marker.id === stepKey) ?? null;
}

function markerAtCoordinate(markers: JourneyMapMarker[], coordinate: MapCoordinate): JourneyMapMarker | null {
...
/** A step's own marker, or the marker at the same coordinate (e.g. a "ride" step has none). */
