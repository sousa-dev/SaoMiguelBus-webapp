import type { MinibusLine, MinibusNetwork, MinibusNetworkLine, MinibusNetworkStop } from '@/lib/types';

import { hasCoordinates, lineMapStops, stopCoordinate } from './stops';
...
function lineMeta(networkLine: MinibusNetworkLine, catalog: MinibusLine | undefined): Omit<MinibusLiveMapStopLine, 'sequence'> {
  return {
...
/** Every stop on the network, one pin per physical location (optionally filtered by line). */
export function liveNetworkMapStops(
  ...
  if (selectedLineSlug) {
    ...
    return lineMapStops(networkLine.stops)
      ...
      .map((stop) => ({
        ...
        lines: [{ ...meta, sequence: stop.sequence }],
      }));
  }
  ...
  for (const networkLine of network.lines) {
    ...
    for (const stop of lineMapStops(networkLine.stops)) {
      ...
      const lineEntry: MinibusLiveMapStopLine = { ...meta, sequence: stop.sequence };
...
export function findLiveMapStopPin(pins: MinibusLiveMapStopPin[], stopKey: string): MinibusLiveMapStopPin | null {
