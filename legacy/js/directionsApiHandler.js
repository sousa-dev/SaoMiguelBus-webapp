...
function searchStepByStep(origin, destination, day, time) {
    ...
    const url = `${API_BASE}/api/v1/gmaps?origin=` + encodeURIComponent(parameters.origin) 
...
function postToStatsStepByStep(parameters) {
    const url = `${API_BASE}/api/v1/stat?request=get_directions&origin=${encodeURIComponent(parameters.origin)}&destination=${encodeURIComponent(parameters.destination)}&time=${encodeURIComponent(parameters.time)}&language=${encodeURIComponent(currentLanguage)}&platform=web&day=${encodeURIComponent(parameters.day)}`;
    fetch(url, {
        method: 'POST',
        headers: {
